...
class BlobConfig(ConfigHolder):
    ...
    def __init__(self, *args, repo_dir=None, **kwargs):
        ConfigHolder.__init__(self, *args, **kwargs)
        setattr(self, "_repo_dir", repo_dir)
        setattr(self, "_repo_dir", repo_dir)
        setattr(self, "_last_fetch", datetime.now())

    def get_dynamic(self, key):
        if self._repo_dir is None or not os.path.isdir(self._repo_dir):
            # if not repo or repo was deleted, just get "cached"
            return getattr(self, key)
        if datetime.now() >= self._last_fetch + timedelta(minutes=5):
            # if more than 5 minutes passed from last fetch, fetch again and get key
            # try:
